...
@dataclass  # shorthand for __init__ setters
class LinearModel:
    weights: np.ndarray
    bias: float
...
class Data:
    model: LinearModel
    rng: InitVar[np.random.Generator]  # Variable only for __init__ & __post_init__
    num_features: int
    num_samples: int
    sigma: float
    x: np.ndarray = field(
        init=False
    )  # Makes it not be included in the implicit __init__
    y: np.ndarray = field(init=False)

    def __post_init__(self, rng):  # dataclass calls it after __init__
